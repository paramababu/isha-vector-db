...
//!
//! The scalar reference lives in `vdb_core::search::metric` and is the definition of what these
...
//!
//! Native-order reinterpretation is only correct on a little-endian target, which `vdb-format`
...
pub fn dot(query: &[f32], row: &[u8]) -> f32 {
    match as_f32(row) {
        ...
        None => vdb_core::search::metric::dot_bytes(query, row),
    }
}
...
pub fn l2_squared(query: &[f32], row: &[u8]) -> f32 {
    match as_f32(row) {
        ...
        None => vdb_core::search::metric::l2_squared_bytes(query, row),
    }
}
...
pub fn dot_f32(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[allow(unreachable_code)]
    vdb_core::search::metric::dot(a, b)
}
...
pub fn l2_squared_f32(a: &[f32], b: &[f32]) -> f32 {
    ...
    #[allow(unreachable_code)]
    vdb_core::search::metric::l2_squared(a, b)
}
