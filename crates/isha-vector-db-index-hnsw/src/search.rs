...
use isha_vector_db_core::index::SearchCtx;
use isha_vector_db_core::search::TopK;
...
fn fallback(ctx: &SearchCtx<'_>, out: &mut TopK) -> Result<()> {
    use isha_vector_db_core::index::VectorIndex;
    isha_vector_db_core::index::ExactScan::new().search(ctx, out)
}
