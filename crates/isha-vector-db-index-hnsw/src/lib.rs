...
//! # What this is for
//!
//! The exact scan in `isha-vector-db-core` compares the query against every live vector. That is the right
//! answer up to a point — it is cache-friendly, it is trivially correct, and with the SIMD
//! kernels in `isha-vector-db-index-flat` it handles tens of thousands of vectors in single-digit
