...
fn the_graph_survives_a_reopen() {
    ...
    let first = {
        let db = Database::open_with_index(
            Arc::clone(&storage) as Arc<dyn isha_vector_db_core::storage::Storage>,
            DatabaseConfig::default(),
            Arc::clone(&clock) as Arc<dyn isha_vector_db_core::clock::Clock>,
            Arc::new(HnswIndex::new()),
        )
        .unwrap();
        let c = db
    ...
    let db = Database::open_with_index(
        ...
        clock,
        Arc::clone(&index) as Arc<dyn isha_vector_db_core::index::VectorIndex>,
    )
    .unwrap();
    let c = db.open_collection("docs").unwrap();
