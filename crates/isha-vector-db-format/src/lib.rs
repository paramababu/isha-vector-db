//! The isha-vector-db on-disk format.
//!
//! This crate is the only place that knows what an isha-vector-db database looks like as bytes. It is
//! separate from `isha-vector-db-core` for three reasons:
//!
...
compile_error!(
    "vdb's on-disk format is little-endian only (see docs/architecture/01-scope.md §1.4). \
     ...
     isha-vector-db-format, and reworking how isha-vector-db-index-flat reads stored vectors."
);
