...
use isha_vector_db_core::{Metric, Result, WriteBatch};
...
fn hnsw_against_flat(scale: Scale, vectors: &[Vec<f32>]) -> Result<Vec<Measurement>> {
    ...
    let mut truth: Vec<Vec<isha_vector_db_core::document::DocId>> = Vec::new();
    let mut approx: Vec<Vec<isha_vector_db_core::document::DocId>> = Vec::new();
    ...
    for (label, dir, use_hnsw) in [
        ...
        let db = if use_hnsw {
            Database::open_with_index(
                ...
                config,
                clock,
                Arc::new(isha_vector_db_index_hnsw::HnswIndex::new()),
            )?
        } else {
            Database::open_with_index(
                storage,
    ...
    recall.note(
        "ef_search",
        isha_vector_db_index_hnsw::HnswParams::default().ef_search as u64,
    );
    out.push(recall);
    Ok(out)
}
...
pub(crate) fn run_all(scale: Scale) -> Result<Vec<Measurement>> {
    ...
    kernel.note(
        "backend",
        isha_vector_db_index_flat::FlatIndex::new().backend().name(),
    );
    out.push(kernel);
    Ok(out)
}
...
fn get_by_id(c: &Collection, scale: Scale) -> Result<Measurement> {
    ...
    let m = sampled("get_by_id", "lookup", scale.queries.min(1000), |i| {
        let id = isha_vector_db_core::DocId::from(format!("doc-{:07}", i % scale.documents));
...
fn scalar_search_for_comparison(
    ...
    m.note("kernel", "scalar (isha-vector-db-core ExactScan)");
    m.note("dimension", scale.dimension);
    db.close()?;
    Ok(m)
}
...
fn recovery_after_unclean_shutdown(scale: Scale, vectors: &[Vec<f32>]) -> Result<Measurement> {
    ...
    if recovered as usize != unflushed {
        return Err(isha_vector_db_core::internal_error!(
