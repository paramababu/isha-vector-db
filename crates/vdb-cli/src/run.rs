...
use vdb_core::api::{CompactOptions, Database, DatabaseConfig, VerifyLevel};
use vdb_core::clock::Clock;
use vdb_core::error::{ConfigError, Result};
use vdb_core::Include;
use vdb_storage_os::OsStorage;

use crate::format::{bytes, count, field, heading};

const USAGE: &str = "\
vdb — inspect, verify and compact a vdb database

USAGE
  vdb <command> <path> [options]
...
pub(crate) fn dispatch(raw: &[String]) -> Result<ExitCode> {
    ...
    match command {
        "version" => {
            println!("vdb {}", env!("CARGO_PKG_VERSION"));
            println!("on-disk format: v{}", vdb_format::FORMAT_VERSION);
            println!(
                "reads formats: v{}..=v{}",
...
fn stats(args: &Args<'_>) -> Result<ExitCode> {
    ...
    if s.total_rows > s.live_documents {
        ...
        field(
            "tombstoned rows",
            format!("{} (run `vdb compact`)", count(dead)),
        );
    }
...
fn compact(args: &Args<'_>) -> Result<ExitCode> {
    ...
    if let Some(raw) = args.value("min-dead") {
        let ratio: f32 = raw.parse().map_err(|_| -> vdb_core::DbError {
            ConfigError::InvalidField {
                field: "--min-dead",
                value: raw.to_owned(),
...
fn get(args: &Args<'_>) -> Result<ExitCode> {
    ...
    let (Some(collection), Some(id)) = (args.at(2), args.at(3)) else {
        return Err(ConfigError::InvalidField {
            field: "arguments",
            value: "<missing>".to_owned(),
            constraint: "vdb get <path> <collection> <id>",
        }
        .into());
    };
    ...
    let doc_id = match c.catalog().id_kind {
        vdb_format::IdKind::U64 => match id.parse::<u64>() {
            Ok(v) => vdb_core::DocId::U64(v),
            Err(_) => {
                return Err(ConfigError::InvalidField {
                    field: "id",
                    value: id.to_owned(),
