...
impl TempDir {
    fn new(label: &str) -> Self {
        ...
        let path = std::env::temp_dir().join(format!("vdb-cli-{label}-{}-{n}", std::process::id()));
...
fn stats_reports_what_is_in_the_database() {
    ...
    assert!(out.contains("vdb compact"), "and the fix suggested: {out}");
}
