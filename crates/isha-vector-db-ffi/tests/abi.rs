...
fn open(dir: &TempDir) -> *mut isha_vector_db_ffi::VdbDb {
    let path = dir.bytes();
...
fn collection(
    db: *mut isha_vector_db_ffi::VdbDb,
    dim: u32,
) -> *mut isha_vector_db_ffi::VdbCollection {
    let name = b"docs";
...
fn message(err: *mut isha_vector_db_ffi::VdbError) -> String {
    if err.is_null() {
...
fn upsert(c: *mut isha_vector_db_ffi::VdbCollection, id: &str, v: &[f32]) {
    let mut err = ptr::null_mut();
    let rc = unsafe {
        vdb_upsert(
            c,
...
fn meta(pairs: &[(&str, &str)], numbers: &[(&str, i64)]) -> *mut isha_vector_db_ffi::VdbMetadata {
    let m = vdb_metadata_new();
    let mut err = ptr::null_mut();
    for (k, v) in pairs {
...
fn filtered_ids(
    c: *mut isha_vector_db_ffi::VdbCollection,
    f: *mut isha_vector_db_ffi::VdbFilter,
) -> Vec<String> {
...
fn stats_compaction_and_verification_work_through_the_abi() {
    ...
    let mut stats = isha_vector_db_ffi::VdbStats::default();
    assert_eq!(
