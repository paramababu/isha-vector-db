...
//! byte.
//!
//! With these symbols linked in, `isha-vector-db-testkit`'s storage conformance suite runs against
...
pub unsafe extern "C" fn vdb_host_open(path: *const u8, path_len: usize, mode: u32) -> i32 {
    ...
    if matches!(fs.nodes.get(path), Some(Node::Dir)) {
        ...
        // rejects; `isha-vector-db-storage-os` had a bug here once and the conformance suite caught it.
