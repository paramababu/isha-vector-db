...
//! engine knows a filesystem exists. That is not a slogan: `isha-vector-db-core` and `isha-vector-db-format` are
...
///
/// This lives here rather than in `isha-vector-db-core` for the same reason `OsStorage` does: it is a
...
impl isha_vector_db_core::clock::Clock for SystemClock {
    fn now_ms(&self) -> u64 {
