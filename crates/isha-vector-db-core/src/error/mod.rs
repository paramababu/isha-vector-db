...
///
/// `isha-vector-db-format` decodes byte slices and has no concept of a file, so the layer that *does* know
...
pub fn from_format(e: isha_vector_db_format::FormatError) -> DbError {
    from_format_at(e, &DbPath::root())
}

/// Convert a format-level decode failure, attaching the file it came from.
pub fn from_format_at(e: isha_vector_db_format::FormatError, path: &DbPath) -> DbError {
    use isha_vector_db_format::FormatError as F;
    match e {
