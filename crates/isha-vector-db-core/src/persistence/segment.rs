...
use isha_vector_db_format::{Catalog, FileKind, SegmentRef};
    ...
    for row in memtable.live_rows() {
        ...
        let record = MetaRecord {
            metadata: row
                .metadata
                .as_ref()
                .map(|m| isha_vector_db_format::Value::Map(m.as_map().clone())),
...
/// collections, and it is what `isha-vector-db-storage-os` will replace with a memory map for the vector
...
impl SegmentData {
    ...
    pub fn verify_checksums(&self, storage: &dyn Storage, collection: &str) -> Result<()> {
        for which in SegmentFile::ALL {
            ...
            isha_vector_db_format::verify_block(&bytes, kind)
                .map_err(|e| from_format_at(e, &path))?;
        }
        Ok(())
    }
    ...
    pub fn metadata(&self, row: u32) -> Result<Metadata> {
        ...
        match record.metadata {
            Some(isha_vector_db_format::Value::Map(m)) => Ok(Metadata::from_map(m)),
            _ => Ok(Metadata::new()),
        }
    }
    ...
    pub fn document(&self, row: u32, include: Include) -> Result<Option<Document>> {
        ...
        if include.metadata || include.content {
            let record = MetaBlock::from_payload(&self.meta_payload)
                ...
                .map_err(|e| from_format_at(e, &DbPath::root()))?;
            if include.metadata {
                if let Some(isha_vector_db_format::Value::Map(m)) = record.metadata {
