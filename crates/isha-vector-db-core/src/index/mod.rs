...
//! suite in `isha-vector-db-testkit`, which any implementation must pass.
...
/// so it lives in `isha-vector-db-core` alongside the trait it implements, for the same reason
/// `isha-vector-db-storage-memory` is the reference storage backend: the thing everything else is compared
/// to should be the simplest possible implementation, in the crate that forbids `unsafe`, with
/// nothing clever in it to be wrong.
///
/// `isha-vector-db-index-flat` is where the SIMD-accelerated version of this scan lives, because that needs
