...
//!
//! `isha-vector-db-core` never opens a file. It asks a [`Storage`] implementation to do it, and that
//! implementation is supplied by the embedder: `isha-vector-db-storage-os` on a desktop or phone,
//! `vdb-storage-opfs` in a browser, `isha-vector-db-storage-memory` in a test. The engine's whole knowledge
...
//! honestly what it declares — `isha-vector-db-testkit`'s conformance suite checks exactly that.
//!
