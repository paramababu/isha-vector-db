...
use isha_vector_db_format::{Catalog, CollectionEntry, Manifest};
...
impl Database {
    ...
    ///
    /// `isha-vector-db-core` cannot depend on an index crate — that inversion is what lets a mobile build
    /// ship only the indexes it needs, and it is why the core can forbid `unsafe` at all. So the
    /// accelerated scan in `isha-vector-db-index-flat`, whose SIMD kernels need `unsafe`, arrives this way:
    ///
    /// ```ignore
    /// Database::open_with_index(storage, config, clock, Arc::new(isha_vector_db_index_flat::FlatIndex::new()))
    /// ```
    ///
    ...
    pub fn stats(&self) -> Result<DatabaseStats> {
        ...
        Ok(DatabaseStats {
            format_version: isha_vector_db_format::FORMAT_VERSION,
...
pub struct CollectionInfo {
    ...
    pub metric: isha_vector_db_format::Metric,
}
