...
use isha_vector_db_core::document::DocId;
use isha_vector_db_core::path::DbPath;
use isha_vector_db_core::persistence::{replay_into, wal::WalWriter, Durability};
...
use isha_vector_db_format::{IdKind, WalOp};
...
fn run_workload(storage: &dyn Storage, durability: Durability) -> isha_vector_db_core::Result<()> {
...
fn recover(storage: &dyn Storage) -> isha_vector_db_core::Result<BTreeSet<String>> {
...
fn replay_reports_what_it_rolled_back() {
    ...
    let mut buf = isha_vector_db_format::Writer::new();
    for (seq, id) in [(10u64, "orphan-1"), (11, "orphan-2")] {
        isha_vector_db_format::WalFrame::in_txn(seq, 99, put(id, [0.0, 0.0]))
            .append_to(&mut buf)
            .unwrap();
    }
