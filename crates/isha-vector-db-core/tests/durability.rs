...
use isha_vector_db_core::document::{DocId, Include};
use isha_vector_db_core::path::DbPath;
use isha_vector_db_core::persistence::segment::{
    flush_memtable, list_segment_ids, read_catalog, remove_segment, write_catalog, SegmentData,
};
use isha_vector_db_core::persistence::{
    layout, replay_into, wal::WalWriter, Durability, ManifestStore,
};
use isha_vector_db_core::storage::Storage;
use isha_vector_db_core::vector::{VectorDType, VectorView};
use isha_vector_db_core::write::Memtable;
...
use isha_vector_db_testkit::{Fault, FaultyStorage};
...
fn create(storage: &dyn Storage) -> isha_vector_db_core::Result<()> {
...
fn reopen(storage: &dyn Storage, clean_orphans: bool) -> isha_vector_db_core::Result<Recovered> {
...
fn write_and_flush(
    ...
    let mut f = storage.open_file(
        &wal_path(),
        isha_vector_db_core::storage::OpenMode::ReadWrite,
    )?;
    f.truncate(0)?;
    f.sync_data()
}
...
mod maintenance {
    ...
    use isha_vector_db_core::document::{DocId, DocumentInput};
    use isha_vector_db_core::metadata::{Metadata, Value};
    ...
    fn populated(
        ...
        total: usize,
        deleted: usize,
    ) -> isha_vector_db_core::api::Collection {
        let c = db
    ...
    fn verify_detects_bit_rot_that_a_quick_check_cannot() {
        ...
        let path = isha_vector_db_core::path::DbPath::parse(&vec_path).unwrap();
    ...
    fn verify_warns_about_orphan_files_without_calling_them_damage() {
        ...
        let ghost =
            isha_vector_db_core::path::DbPath::parse("collections/docs/segments/000099.vec")
                .unwrap();
