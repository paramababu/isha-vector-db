...
use isha_vector_db_format::Metric;
...
fn opening_an_empty_storage_creates_a_database() {
    ...
    assert_eq!(stats.format_version, isha_vector_db_format::FORMAT_VERSION);
...
fn a_missing_manifest_over_existing_collections_is_reported_not_overwritten() {
    ...
    for slot in ["MANIFEST-A", "MANIFEST-B"] {
        let path = isha_vector_db_core::path::DbPath::parse(slot).unwrap();
