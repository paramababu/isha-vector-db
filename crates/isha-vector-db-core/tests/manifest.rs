...
//!
//! These live here rather than beside the code because `isha-vector-db-storage-memory` is a dev-dependency
//! cycle back onto `isha-vector-db-core`: the lib's own unit-test build would see two copies of the crate
...
use isha_vector_db_core::persistence::{layout, ManifestStore};
use isha_vector_db_core::storage::Storage as _;
use isha_vector_db_format::{CollectionEntry, Slot, SlotStatus};
