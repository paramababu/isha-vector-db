...
use isha_vector_db_core::api::{
    Collection, CollectionSpec, Database, DatabaseConfig, SearchRequest,
};
use isha_vector_db_core::clock::ManualClock;
use isha_vector_db_core::document::{DocId, DocumentInput, Include};
use isha_vector_db_core::index::{Budget, IndexKind};
...
fn ids(response: &isha_vector_db_core::api::SearchResponse) -> Vec<String> {
    response.hits.iter().map(|h| h.id.display()).collect()
}

fn query(c: &Collection, v: &[f32], k: usize) -> isha_vector_db_core::api::SearchResponse {
...
fn a_cancelled_search_reports_cancellation() {
    ...
    assert!(matches!(
        ...
        Err(isha_vector_db_core::DbError::Cancelled)
    ));
    db.close().unwrap();
}
