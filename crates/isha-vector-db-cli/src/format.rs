...
use isha_vector_db_core::error::Recoverability;
use isha_vector_db_core::DbError;
...
pub(crate) fn advice(e: &DbError) -> &'static str {
    ...
    match e.code() {
        isha_vector_db_core::ErrorCode::DATABASE_NOT_FOUND => {
            return "check the path; this command does not create a database"
        }
        isha_vector_db_core::ErrorCode::DATABASE_ALREADY_OPEN => {
    ...
    match e.recoverability() {
        ...
        Recoverability::NeedsRepair => {
            "run `isha-vector-db verify --full <path>` for the full picture before changing anything"
        }
...
mod tests {
    ...
    fn the_common_cli_errors_get_specific_advice() {
        use isha_vector_db_core::error::LifecycleError;
    ...
    fn every_recoverability_has_advice() {
        use isha_vector_db_core::error::{LifecycleError, ValidationError};
        let cases: Vec<DbError> = vec![
            ...
            isha_vector_db_core::error::CorruptionError::MissingSegment {
                collection: "c".into(),
                segment: 1,
            }
            .into(),
