...
```bash
npm install @isais-logic/isha-vector-db-node
```
...
**This is not the WebAssembly build.** `@isais-logic/isha-vector-db-node` is a native addon and is the right choice on a
