...
def _library_name() -> str:
    ...
    if system == "Darwin":
        return "libisha_vector_db_ffi.dylib"
    if system == "Windows":
        return "isha_vector_db_ffi.dll"
    return "libisha_vector_db_ffi.so"


def _candidates() -> list[Path]:
    """Where to look for the shared library, nearest first.

    ``ISHA_VECTOR_DB_LIBRARY`` wins, because a developer working on the engine needs to point at a build
    they just made without reinstalling anything.
    """
    override = os.environ.get("ISHA_VECTOR_DB_LIBRARY")
    if override:
...
def load() -> ctypes.CDLL:
    ...
    raise ImportError(
        "could not load the isha-vector-db engine. Tried:\n  "
        + tried
        + "\n\nBuild it with `cargo build -p isha-vector-db-ffi --release`, or set ISHA_VECTOR_DB_LIBRARY to the "
...
if sys.version_info < (3, 9):  # pragma: no cover - the package metadata refuses to install
    raise RuntimeError("isha-vector-db requires Python 3.9 or newer")
